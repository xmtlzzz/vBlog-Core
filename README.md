...
- **页脚 GitHub 二维码徽章**：基于 [every-qrcode](https://github.com/AlbertAZ1992/every-qrcode) 的动态徽章，作者 GitHub 链接确定性生成 3D 模型（樱花树/地形 + 氛围特效 + 配色预设）；点击徽章放大并变形为可扫描二维码（内容即 GitHub 主页），再点恢复。WebGPU 不可用时自动降级为静态二维码。后台「设置 → 页脚二维码徽章」可配置启用/模型/特效/配色/背景色，徽章懒加载不影响首屏

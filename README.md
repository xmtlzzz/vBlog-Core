...
- **页脚 GitHub 二维码徽章**：基于 [every-qrcode](https://github.com/AlbertAZ1992/every-qrcode) 的动态徽章，作者 GitHub 链接确定性生成 3D 模型（樱花树/地形 + 氛围特效 + 配色预设 + 尺寸可调）；点击徽章放大并变形为可扫描二维码（内容即 GitHub 主页），再点恢复。WebGPU 不可用时自动降级为静态二维码。后台「设置 → 页脚二维码徽章」可配置启用/模型/特效/配色/尺寸/背景色，徽章懒加载不影响首屏

- **项目二维码导航**：后台配置项目地址列表（每行「名称|URL」），「模块」页为每个地址生成专属 3D 徽章卡片，点击变形为可扫描二维码

- **首页标语**：打字机大标题与副标题均读后台设置，支持多行
...
- **系统设置**：站点信息、首页标语、关于页技术栈、作者信息、页脚二维码徽章（启用/模型/特效/配色/背景色）、功能开关、gRPC API Key 管理
